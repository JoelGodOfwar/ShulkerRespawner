...
1.14	1_14_R1
1.15	1_15_R1
1.16.1	1_16_R1		1.16.2	1_16_R2		1.16.4	1_16_R3
1.17	1_17_R1
*/

public class CSEHandler_1_13_1 implements Listener {
	ShulkerRespawner SR;
	boolean debug;
	
	@SuppressWarnings("static-access")
	public CSEHandler_1_13_1(final ShulkerRespawner plugin){
		SR = plugin;
		debug = plugin.debug;
	}
	...
	public void onCreatureSpawn(CreatureSpawnEvent event){ //onEntitySpawn(EntitySpawnEvent e) {
        ...
        if(debug){SR.logDebug("CSE 1.13 entity=" + entity.getType());}
	...
	public boolean playerInsideStructure(Entity p, StructureType type, String nmsType) {
        ...
        if (chunk.a(nmsType) != null) { //Checking if this chunk is the starting point of the structure
            for (StructurePiece piece : chunk.a(nmsType).d()) { //Iterating through every piece of the structure
                if (piece.d().b(new BaseBlockPosition(p.getLocation().getBlockX(), p.getLocation().getY(), p.getLocation().getBlockZ()))) { //Getting the piece's bounding box and then checking if the player is inside
