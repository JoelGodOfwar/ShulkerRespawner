...
import com.github.joelgodofwar.srl.ShulkerRespawnerLib;
/**
        		...
        		if(entity.getLocation().subtract(0, 3, 0).getBlock().getType().toString().contains("PURPUR")||
        			...
        			if(!checkradius(entity, SR.getConfig().getInt("radius_between_spawns", 10))){ //5
	        			...
	        			if(endcity != null){
	        				...
	        				if(SR.getServer().getPluginManager().getPlugin("ShulkerRespawnerLib") != null){
								if(!ShulkerRespawnerLib.playerInsideStructure(entity)) {
								//if(!playerInsideStructure(entity, StructureType.END_CITY, StructureGenerator.p)) {
									if(debug){SR.logDebug("Enderman is not in an EndCity.");}
									return;
								}
	        				}else {
	        					SR.logWarn("ShulkerRespawnerLib is not loaded, restart server to load it.");
	        					return;
	        				}
